...
const saveVideo = async function (aweme_id, preview_title, video_url) {
  ...
  return new Promise((resolve) => {
    try {
      Console.log(`${preview_title}.mp4开始下载`, 'BgCyan');
      ...
      http.get(video_url, options, function (response) {
        ...
        response.on('data', function (chunk) {
          ...
          if (!mp.get(trunc) && trunc && !(trunc % 10)) {
            ...
            Console.log(`${preview_title}.mp4已下载[${progress}%]`, 'BgYellow');
          }
          if (!mp.get(trunc)) {
            mp.set(trunc, true);
          }
        ...
        file.on('finish', async () => {
          file.close();
          mp.clear();
          Console.log(`${preview_title}.mp4下载完成`);
          resolve();
        });
      });
    } catch (error) {
      Console.log(`${preview_title}.mp4下载失败:${error}`, 'BgRed');
      resolve();
    }
  });
};
...
const saveCursorList = function (cursor_list) {
  try {
    ...
    Console.log(`保存cursor_list出错:${err}`, 'BgRed');
  }
};
...
const readCursorList = function () {
  try {
    ...
    Console.log(`读取cursor_list出错:${err}`, 'BgRed');
  }
  return [];
};
  ...
  return new Promise(async (resolve) => {
    try {
      ...
      for (let i = 0; i < len; ++i) {
        ...
        if (
          query_has_res?.length &&
          query_has_res?.[0] &&
          query_has_res?.[0][query_has_child_sql]
        ) {
      ...
      Console.log(`SQL文件保存在${save_sql_path}`);
      await runSql(sql);
    } catch (err) {
      Console.log('保存SQL出错:' + err, 'BgRed');
    }
    return resolve();
  });
};
...
const run = async function () {
  ...
  const cursor_list = [];
  const last_cursor_list = readCursorList();
  const last_cursor_list_last = last_cursor_list.length
    ? last_cursor_list[last_cursor_list.length - 1]
    : 0;
    ...
    try {
      ...
      while (1) {
        ...
        if (!res?.aweme_list?.length) {
          if (!cursor) {
            Console.log(
              `请重新填写${app_path}${app_config_path}中的cookie后重新运行`
            );
        ...
        await upAndSaveSql(cursor_list);
        if (
          get_video_cnt > update_list_length_limit &&
          has_match_last_cursor_list_last
        ) {
          return resolve(true);
        }
        last_cursor = cursor;
      ...
      Console.log(`下载失败:${error}`, 'BgRed');
    }
    Console.log(`抖音收藏爬取完成`);
    await upAndSaveSql(cursor_list);
    return resolve(true);
  });
};
